...
# Loader for the Biwi Kinect Head Pose Database (sequences in data/biwi/NN/).
#
# RGB and depth come from the SAME Kinect with a calibrated extrinsic between
# the two cameras (rgb.cal / depth.cal per sequence), so the sparse
# RGB-landmark term and the dense depth term share one camera system.
#
# Conventions (verified visually on seq 01, see python/check_biwi.py):
#   - depth .bin  : 640x480 uint16 millimetres, run-length encoded (see
#                   read_depth_bin). Background is zeroed; only the person
#                   remains (~18% of pixels).
#   - depth camera = world frame (depth.cal has identity R, zero t).
#   - rgb.cal R, t map depth-cam points INTO the RGB camera:
#                   p_rgb = R @ p_depth + t          (t in millimetres)
#     (NOT the inverse — projecting with the inverse lands ~20 px off.)
#   - axes are OpenCV-style: +X right, +Y DOWN, +Z forward — the same frame
#     as the C++ side (backprojectDepth / proj::BFM_TO_CAM). Everything in mm.
#   - frame_XXXXX_pose.txt: ground-truth head rotation (3x3) and head centre
#     (mm, depth cam). Free head-crop centre + pose evaluation for nothing.
# =============================================================================
import glob
import os
import re
...
def read_depth_bin(path: str) -> np.ndarray:
    """Decode one Biwi RLE depth file into a (480, 640) uint16 array of mm.

    Format: int32 width, int32 height, then repeated
    (int32 num_empty, int32 num_full, num_full * int16 depth values):
    num_empty pixels are zero (background), num_full carry values, row-major.
    """
...
def read_cal(path: str):
    """Parse a Biwi .cal file -> (K 3x3, dist 4, R 3x3, t 3, (width, height))."""
...
class BiwiFrame:
    """One Biwi frame: RGB, depth (mm) and ground-truth head pose."""
...
def backproject_mm(depth_mm: np.ndarray, K: np.ndarray) -> np.ndarray:
    """Depth image -> Nx3 points, millimetres, depth-cam frame (+Y down)."""
...
def depth_to_rgb_cam(points_mm: np.ndarray, R: np.ndarray, t: np.ndarray) -> np.ndarray:
    """Depth-cam points (mm) -> RGB-cam points (mm). Verified: p' = R p + t."""
    return points_mm @ R.T + t


def project(points_mm: np.ndarray, K: np.ndarray) -> np.ndarray:
    """Pinhole projection of Nx3 camera-frame points -> Nx2 pixels."""
...
def crop_head_mm(points_mm: np.ndarray, center_mm: np.ndarray,
    ...
    """Head crop mirroring the C++ cropHead (tight radius per dense-fit notes)."""
