                              ...
                              const std::vector<Eigen::Vector3d>* depthCloud)
{
    ...
    // Appearance: albedo β + SH lighting via the photometric fit (one-off).
    ...
    beta_     = photo.albedoCoefficients;
    prevPose_ = geo.pose;      // geometry pose is the tracking reference
    prevSh_   = photo.sh;
                        ...
                        const std::vector<Eigen::Vector3d>* depthCloud)
{
    ...
    if (havePrev2_) {
        ...
        initExpr = (2.0 * prevExpr_ - prev2Expr_).cwiseMax(-3.0).cwiseMin(3.0);
    }
    ...
    const double a = cfg_.smoothAlpha;
