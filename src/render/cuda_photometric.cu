...
// finite-difference step for each kind of parameter, small enough to stay
// inside a texel so the gradient is local but big enough to avoid float noise
...
// rotate a point by an angle-axis vector, with a small-angle branch
...
// bilinear sample of the image, clamped at the border
...
// same bilinear sample but also returns the image gradient dI/du and dI/dv
// used by the analytic jacobian path, it is the exact derivative of the
// bilinear interpolant and is 0 in the direction we clamped at a border
...
// the rgb residual for one pixel given the params (aa, t, shape)
...
// pick the finite-diff step for parameter a, the layout matches the host
...
// kernel that sums up JtJ, Jtr and the huber cost over all pixels
// analytic==0 uses the finite-difference jacobian, perturbing the global params
// analytic==1 uses the analytic jacobian (image gradient times projection times
//             the pose/shape chain). for rotation it perturbs locally with a
//             small delta so d cam / d delta = -[rot]_x, and the host puts that
//             back together as R(delta) * R_cur
                               ...
                               double* JtJ, double* Jtr, double* cost)
{
    ...
    // huber reweighting on the rgb block, s2 is the squared residual length
    const double s2  = r0[0] * r0[0] + r0[1] * r0[1] + r0[2] * r0[2];
    const double d2  = huberDelta * huberDelta;
    const double w   = (s2 <= d2) ? 1.0 : sqrt(huberDelta / sqrt(s2));  // sqrt of the huber weight
    const double rho = (s2 <= d2) ? s2 : (2.0 * huberDelta * sqrt(s2) - d2);
    atomicAdd(cost, rho);

    double J[3 * MAXNP];
    if (analytic) {
        // recompute the geometry at the current params, this is where we linearise
        double S[3] = { base[0], base[1], base[2] };
        ...
        // how the projected u,v move when the camera-space point moves
        const double duX = s.fx / Z, duZ = -s.fx * X / (Z * Z);
        const double dvY = s.fy / Z, dvZ = -s.fy * Y / (Z * Z);
        const double wsq = -w * sqrtW;   // huber weight times residual scale times -1

        int a = 0;
        if (optimizePose) {
            // local rotation delta, the columns of d cam / d delta are -[rot]_x
            const double dc[3][3] = {
                {  0.0,     -rot[2],  rot[1] },
                {  rot[2],   0.0,    -rot[0] },
                { -rot[1],   rot[0],  0.0    } };
            ...
            // translation, moving t_j just moves the camera point along that axis
            for (int j = 0; j < 3; ++j) {
        ...
        if (s.optimizeShape) {
            for (int k = 0; k < s.K; ++k) {
                ...
                double rb[3];
                aaRotate(aa, bk, rb);                     // d cam / d shape_k is R_cur times basis_k
...
// kernel that only sums the huber cost, used when LM just needs to test a step
...
// uploads (aa,t,shape) to the device, runs normalEqKernel, reads back JtJ/Jtr/cost
                       ...
                       double* JtJ, double* Jtr, double* cost)
{
    ...
    (void)optimizeShape;  // already fixed when we called prepare()
