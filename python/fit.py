# analysis-by-synthesis fit, a proof of concept, mirrors the C++ Optimizer
#
# it fits the BFM to the depth point cloud with iterative closest point plus
# Levenberg-Marquardt
#
...
#   2. for every target point find the nearest model vertex (the ICP matches)
#   3. run Levenberg-Marquardt to shrink ||model - target||^2 + reg*||alpha||^2
#
# the photometric term and expression are left out on purpose, those come later
# this is enough to show the loop of predicting the face parameters over N steps
import numpy as np
...
def axis_angle_to_R(r):
    """Rodrigues formula, turns an axis-angle vector (3,) into a rotation matrix (3, 3)"""
...
class Fitter:
    def __init__(self, bfm, n_shape=30, reg=2.0, n_model_pts=6000,
        ...
        # take fewer model vertices for speed, the full mesh is 53k
        stride = max(1, bfm.n_vertices // n_model_pts)
        sub = np.arange(0, bfm.n_vertices, stride)
        rows = (3 * sub[:, None] + np.array([0, 1, 2])).reshape(-1)  # (3S,)

        # everything is in mm (the BFM's native unit) and centred on the mean-face
        # centroid, working in mm keeps the regulariser meaningful, reg is roughly
        # the depth noise in mm squared
        ...
        # 180 deg about Y, the BFM looks +z but we want the face looking at the camera (-z)
        self.R_init = np.diag([-1.0, 1.0, -1.0])

    # builds the world-space vertices from the parameter vector p
    ...
    # apply the current pose (rotation, scale, translation) to some 3D points
    ...
    # runs the whole fit and returns the final parameter vector p
